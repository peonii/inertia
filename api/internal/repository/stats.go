...
func (r *PostgresUserStatsRepository) Get(ctx context.Context, userID string) (*domain.UserStats, error) {
	query := `
	SELECT id, user_id, xp, wins, losses, draws, games, quests, events, powerups, catches, times_caught, created_at FROM user_stats WHERE user_id = $1
	`
...
func (r *PostgresUserStatsRepository) GetTop(ctx context.Context, cursor int) ([]*domain.UserStats, error) {
	query := `
	SELECT id, user_id, xp, wins, losses, draws, games, quests, events, powerups, catches, times_caught, created_at FROM user_stats ORDER BY xp DESC LIMIT 10 OFFSET $1
	`
...
func (r *PostgresUserStatsRepository) Update(ctx context.Context, userID string, stats *domain.UserStats) error {
	query := `
	UPDATE user_stats SET xp = $1, wins = $2, losses = $3, draws = $4, games = $5, quests = $6, events = $7, powerups = $8, catches = $9, times_caught = $10 WHERE user_id = $11
	`

	_, err := r.db.Exec(ctx, query, stats.XP, stats.Wins, stats.Losses, stats.Draws, stats.Games, stats.Quests, stats.Events, stats.Powerups, stats.Catches, stats.TimesCaught, userID)
	if err != nil {
		return err
	}
