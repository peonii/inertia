...
func (r *PostgresTeamRepository) Update(ctx context.Context, id string, team *domain.TeamUpdate) (*domain.Team, error) {
	args := make([]interface{}, 0)
	args = append(args, id)
	qtext, args := GatherFields(team, 2, args)

	query := fmt.Sprintf(`
		UPDATE teams SET %s WHERE id = $1
		RETURNING id, name, xp, balance, emoji, color, is_runner, veto_period_end, game_id, created_at
	`, qtext)
